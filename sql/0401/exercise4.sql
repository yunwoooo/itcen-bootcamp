...
--	20번 부서    4명
--	30번 부서    6명
select IF(DEPTNO, concat(DEPTNO, '번 부서'),'미정') '부서정보', concat(count(ENAME), '명') '직원수' from emp group by DEPTNO;
...
--	1980년	   1명
--	1983년	   1명
select year(HIREDATE) '입사년도', concat(count(ENAME), '명') '입사직원수' from emp group by year(HIREDATE) order by count(ENAME) desc;
...
--	SALESMAN          	5,600
--	 ANAYST	  	6,000
select JOB '직급명', format(sum(SAL), 0) '총액' from emp where JOB != 'MANAGER' group by JOB having sum(SAL) > 5000;
...
--	  	MANAGER			2850.00
--	  	CLERK			950.00
select JOB '직무', round(avg(SAL + ifnull(COMM, 0)), 2) '평균급여' from emp where DEPTNO = '30' group by JOB;
...
--	9	  2명
--	10	  3명
select month(HIREDATE) '입사월', concat(count(ENAME), '명') '인원' from emp group by month(HIREDATE) having count(ENAME) >= 2 order by month(HIREDATE);
...
-- MANAGER       8275 
-- ANALYST        6000 
select JOB '직무', sum(SAL) '급여의 합' from emp group by JOB having sum(SAL) > 5000;
...
-- SALESMAN     5600 
-- PRESIDENT    5000 
select JOB '직무', sum(SAL) '급여의 합' from emp where year(HIREDATE) = '1981' group by JOB having sum(SAL) > 3000 order by sum(SAL) desc;
